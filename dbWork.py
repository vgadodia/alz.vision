...
@app.route('/redescription')
def redescription():
    image = db.find_one_or_404({"name": NAME})
    print(image)
    image["memories"][0]["file"] # Access a random file
    return render_template('redescription.html', image=str(pathlib.Path(__file__).parent.absolute()) + "/static/" + image)
